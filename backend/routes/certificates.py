...
def issue_certificate():
    ...
    qr_data = f"{os.getenv('FRONTEND_URL', 'http://127.0.0.1:5173')}/verify/{cert.certificate_number}"
    ...
    draw_centered_text(c, 200, 180, "_________________________")
    draw_centered_text(c, 200, 160, "Registrar")
    draw_centered_text(c, 200, 140, "Puntland State University")
    
    draw_centered_text(c, width-200, 180, "_________________________")
    draw_centered_text(c, width-200, 160, "Dean of Faculty")
    draw_centered_text(c, width-200, 140, "Puntland State University")
    
    # Footer
    c.setFillColor(psu_blue)
    c.setFont("Helvetica-Oblique", 10)
    draw_centered_text(c, width/2, 60, "This certificate is issued by Puntland State University and is valid upon verification")
...
# ---------------------------------------------------------
@cert_bp.route("/verify/<int:id>", methods=["GET"])
...
# ---------------------------------------------------------
# 3b. VERIFY CERTIFICATE BY CERTIFICATE NUMBER (Public)
# ---------------------------------------------------------
...
def download_certificate(id):
    ...
    if not cert.pdf_path:
        ...
        if not cert.qrcode_path:
            qr_filename = f"{cert.id}.png"
            qr_path = os.path.join(QR_DIR, qr_filename)
            qr_data = f"http://127.0.0.1:5173/verify/{cert.certificate_number}"
            img = qrcode.make(qr_data)
            img.save(qr_path)
            cert.qrcode_path = qr_filename
        ...
        c.setFillColor(psu_blue)
        c.setFont("Helvetica-Bold", 24)
        draw_centered_text(c, width/2, height-330, cert.student_name.upper())
        ...
        c.setFillColor(dark_blue)
        c.setFont("Helvetica-Bold", 20)
        draw_centered_text(c, width/2, height-410, cert.course.title)
        
        # Date and certificate info
        c.setFillColor(black)
        c.setFont("Helvetica", 14)
        c.drawString(100, height-480, f"Date of Issue: {cert.issue_date.strftime('%B %d, %Y')}")
        c.drawString(100, height-500, f"Student ID: {cert.student_id}")
        c.drawString(100, height-520, f"Certificate No: {cert.certificate_number}")
        
        # QR Code
        qr_full_path = os.path.join(QR_DIR, cert.qrcode_path)
        if os.path.exists(qr_full_path):
            c.drawImage(qr_full_path, width-200, 100, width=120, height=120)
            c.setFont("Helvetica", 10)
            draw_centered_text(c, width-140, 80, "Scan to Verify")
